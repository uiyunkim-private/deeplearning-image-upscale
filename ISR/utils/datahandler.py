...
class DataHandler:
    ...
    def correct_hr_dim(self, lr_img, hr_img, scale):
        ...
        returns:
        corrected_img/hr_img -- numpy array of truncated/padded/unchanged hr_img
        """

        if (lr_img.shape[0] * scale, lr_img.shape[1] * scale) == (hr_img.shape[0], hr_img.shape[1]):
            return hr_img  # if dimension are already correct return same as it is.
        else:

            if lr_img.ndim == 3:
                # color image
                new_img = np.zeros((lr_img.shape[0] * scale, lr_img.shape[1] * scale, 3))
                new_img[0:hr_img.shape[0], 0:hr_img.shape[1]] = hr_img[0:new_img.shape[0]
                , 0:new_img.shape[1]]

            if lr_img.ndim == 2:
                # Gray scale image
                new_img = np.zeros((lr_img.shape[0] * scale, lr_img.shape[1] * scale))
                new_img[0:hr_img.shape[0], 0:hr_img.shape[1]] = hr_img[0:new_img.shape[0]
                , 0:new_img.shape[1]]
